"""
Run QuestaSim simulation for Round-Robin Arbiter UVM Verification.
...
try:
    parser = argparse.ArgumentParser(description="Run QuestaSim simulation for Round-Robin Arbiter UVM Verification")
    ...
    parser.add_argument('--test', type=str, default='RrBaseTest4', help="UVM Test name.")
    parser.add_argument('--timeout', type=int, default=300, help="Simulation timeout in seconds.")
    parser.add_argument('--verbosity', type=str, default='UVM_HIGH', 
                        choices=['UVM_NONE', 'UVM_LOW', 'UVM_MEDIUM', 'UVM_HIGH', 'UVM_FULL', 'UVM_DEBUG'],
                        help="UVM verbosity level (default: UVM_HIGH).")
    ...
    cmd = f"vsim -L mtiUvm tb_top_opt +UVM_TESTNAME={args.test} +UVM_VERBOSITY={args.verbosity} -voptargs=+acc -sv_seed {args.seed} "

    coverage_do = ""
    if args.coverage_report:
        cmd += "-coverage "
        rel_ucdb = ucdb_file.relative_to(project_root).as_posix()
        coverage_do = f'coverage save -onexit {rel_ucdb}; '
    ...
    if args.gui:
        ...
        cmd += f' -do "{coverage_do}run -all; quit -f"'
    ...
    if args.coverage_report and ucdb_file.exists():
